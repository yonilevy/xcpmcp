...
**RemoveSwiftPackageHandler** — Identifies the package by `url` (remote) or `local_path` (local, matching either an `XCLocalSwiftPackageReference` by `relativePath` or a folder-reference `PBXFileReference` by `path`). Remote product dependencies are matched via their `package` link. Local product dependencies have **no** link back to their package, so to avoid unlinking unrelated local packages' products, when no `products` filter is given the handler resolves *this* package's own products from its manifest (`swift package dump-package`); if that can't be evaluated it only proceeds when the project has a single local package, otherwise it errors and asks for explicit `products`. With `--target` it only unlinks from that target; without it, unlinks from all targets and removes the package declaration (the package-reference object, or the folder reference detached from its group). Orphaned `XCSwiftPackageProductDependency`/`PBXBuildFile` objects are deleted (XcodeProj serializes every object it holds, so detached objects must be explicitly removed). After writing it runs `SwiftPackageHelpers.stripOrphanedLocalPackageReferences` (see below).
...
- **XcodeProj bug workaround:** `PBXObjects.delete` has no branch for `XCLocalSwiftPackageReference` (it handles the remote and product-dependency types but not the local one), so `pbxproj.delete(object:)` on a local package reference silently leaves the object in the file even though its `packageReferences` entry is gone. `SwiftPackageHelpers.stripOrphanedLocalPackageReferences` post-processes the written pbxproj text, removing any `XCLocalSwiftPackageReference` object block whose id occurs only once (i.e. defined but referenced nowhere) — which also cleans up orphans left in projects damaged by earlier runs.
- Local-package removal resolves the package's products via `swift package dump-package` (rather than guessing) because local product dependencies carry no link to their package; this is the one place the tool shells out, and it degrades safely (single-package match-all, or an explicit-`products` error) when the manifest can't be evaluated.
